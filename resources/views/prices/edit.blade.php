            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('prices.update', $price->id) }}">
                    ...
                    <div class="flex flex-row shadow overflow-hidden sm:rounded-md bg-white dark:bg-stone-600">
                        <div class="px-4 py-5 bg-white dark:bg-stone-600 sm:p-6">
                            ...
                            <select name="market" id="market" class="form-control select2 rounded-md shadow-sm mt-1 block w-full">
                                ...
                                <option {{ ($price->market) == 'Export' ? 'selected' : '' }} value="Export">
                                        Export
                                </option>
                                <option {{ ($price->market) == 'Local Processing' ? 'selected' : '' }} value="Local Processing">
                                    Local Processing
                                </option>
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white dark:bg-stone-600 sm:p-6">
                            ...
                            <select name="species_id" id="specie" class="form-control select2 rounded-md shadow-sm mt-1 block w-full">
                                @foreach($species as $id => $specie)                                    
                                    <option value="{{ $id }}" {{ (isset($price) && $price->species ? $price->species->id : old('species_id')) == $id ? 'selected' : '' }}>
                                        {{ $specie }}
                                    </option>
                            @endforeach
                            </select>
                    ...
                    <div class="flex flex-row shadow overflow-hidden sm:rounded-md bg-white dark:bg-stone-600">
                        ...
                        <div class="px-4 py-5 bg-white dark:bg-stone-600 sm:p-6">
                            ...
                            <select name="method" id="method" class="form-control select2 rounded-md shadow-sm mt-1 block w-full">
                                    ...
                                    <option {{ ($price->method) == 'Non-RIL' ? 'selected' : '' }} value="Non-RIL">
                                        Non-RIL
                                    </option>
                                    <option {{ ($price->method) == 'RIL' ? 'selected' : '' }} value="RIL">
                                        RIL
                                    </option>
                                    <option {{ ($price->method) == 'Helicopter' ? 'selected' : '' }} value="Helicopter">
                                        Helicopter
                                    </option>
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white dark:bg-stone-600 sm:p-6">
                            ...
                            <select name="log_size_id" id="log_size_id" class="form-control select2 rounded-md shadow-sm mt-1 block w-full">
                                @foreach($logsizes as $id => $logsize)
                                    <option value="{{ $id }}" {{ (isset($price) && $price->logsize ? $price->logsize->id : old('log_size_id')) == $id ? 'selected' : '' }}>
                                        {{ $logsize }}
                                    </option>
                            @endforeach
                            </select>
                        ...
                        <div class="px-4 py-5 bg-white dark:bg-stone-600 sm:p-6">
                            <label for="price" class="block font-medium text-sm text-gray-700 dark:text-gray-200">Price</label>
                    ...
                    <div class="flex items-center justify-end px-4 py-3 bg-gray-50 dark:bg-stone-600 text-right sm:px-6">
                        <a href="{{ route('prices.index') }}"  class="pr-4">
                    ...
                    {{-- <div class="flex flex-row shadow overflow-hidden sm:rounded-md bg-white dark:bg-stone-600">
                        <div class="flex items-center justify-end py-3 text-right  px-3">
                            <div class="block mt-8">
                                <a href="{{ route('prices.index') }}" class="bg-gray-200 hover:bg-gray-300 text-black font-bold py-2 px-4 rounded">Cancel</a>
                            </div>
                        </div>
