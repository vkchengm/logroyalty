                                ...
                                @foreach ($licensees as $licensee)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">
                                            @foreach ($licensee->licenses as $license)
                                                ...
                                                @foreach ($license->licenseAccCoupes as $licenseAccCoupe)
                                                    {{ '-'.$licenseAccCoupe->account_no.' '.$licenseAccCoupe->coupe_no }} <br>
                                                @endforeach
                                                
                                                {{-- @foreach ($licensee->licenses->licenseAccCoupes as $licenseAccCoupe) --}}
                                                    {{-- {{ $licensee->license->licenseAccCoupes->account_no }}  --}}
                                                {{-- @endforeach --}}
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            ...
                                            <a href="{{ route('licensees.edit', $licensee->id) }}" class="text-indigo-600 hover:text-indigo-900 mb-2 mr-2">Edit</a>
                                            <form class="inline-block" action="{{ route('licensees.destroy', $licensee->id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
