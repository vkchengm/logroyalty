    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('licensees.update', $licensee->id) }}">
                    @csrf
                    @method('put')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        <div class="px-4 py-5 bg-white dark:bg-stone-600  sm:p-6">
                            ...
                            <select name="type" id="type" class="form-control select2 rounded-md shadow-sm mt-1 block w-full">
                                ...
                                <option {{ ($licensee->type) == 'Company' ? 'selected' : '' }} value="Company">
                                    Company
                                </option>
                                <option {{ ($licensee->type) == 'Individual' ? 'selected' : '' }} value="Individual">
                                    Individual
                                </option>
                                <option {{ ($licensee->type) == 'Group' ? 'selected' : '' }} value="Group">
                                    Group
                                </option>
                            </select>
                            @if($errors->has('type'))
                        ...
                        <div class="px-4 py-5 bg-white dark:bg-stone-600 sm:p-6">
                            <label for="contact_no" class="block font-medium text-sm text-gray-700 dark:text-gray-200">Contact No.</label>
                            <input type="text" name="contact_no" id="contact_no" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('contact_no', $licensee->contact_no) }}" />
                            @error('contact_no')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
