    ...
    <div>
        <div class="max-w-4xl mx-auto py-10 sm:px-6 lg:px-8">
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form method="post" action="{{ route('users.updateexternal', $user->id) }}">
                    @csrf
                    @method('put')
                    <div class="shadow overflow-hidden sm:rounded-md">
                        ...
                        {{-- <div class="px-4 py-5 bg-white dark:bg-stone-600 sm:p-6">
                            <label for="licensee" class="block font-medium text-sm text-gray-700 dark:text-gray-200">licensee</label>
                            <input type="text" name="licensee_id" id="licensee_id" class="form-input rounded-md shadow-sm mt-1 block w-full"
                                   value="{{ old('licensee_id', $user->licensee_id) }}" />
                            @error('licensee_id')
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div> --}}
                        ...
                        <div class="px-4 py-5 bg-white dark:bg-stone-600 sm:p-6">
                            <label for="licensee" class="block font-medium text-sm text-gray-700 dark:text-gray-200">Licensee</label>
                            <select name="licensee_id" id="licensee_id" class="form-control select2 rounded-md shadow-sm mt-1 block w-full">
                                @foreach($licensees as $id => $licensee)
                                    <option value="{{ $id }}" {{ (isset($licensee) && $user->licensee ? $user->licensee->id : old('licensee_id')) == $id ? 'selected' : '' }}>
                                        {{ $licensee }}
                                    </option>
                            @endforeach
                            </select>
                        ...
                        <div class="flex items-center justify-end px-4 py-3 bg-gray-50 dark:bg-stone-600 text-right sm:px-6 gap-4">
                            <a href="{{ route('users.index-external') }}" >
